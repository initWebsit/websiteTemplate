...
export function withDefaultSettings(instance) {
  instance.defaults.baseURL = api_help + '/blogservice'
  instance.defaults.timeout = ajaxTimeout
  instance.mockDefaults.baseURL = '/blogservice'
      ...
      if (/^\/go\/ps/.test(cfg.url) && $q.is.client) {
        ...
        try {
          ...
          if ($_.isObject(clientCommonParams)) {
            ...
            $_.forEach(clientCommonParams, (v, k) => $_.set(cfg.headers, `${k}`, v))
          }
          // 获取正式环境访问sign权限
          ...
          cfg.url = cfg.url.indexOf('?') !== -1 ? cfg.url + `&${urlSign}` : cfg.url + `?${urlSign}`
        } catch (err) {
          console.log(err)
        }
...
export const get = _ajax.get.bind(_ajax)
export const post = _ajax.post.bind(_ajax)
export const put = _ajax.put.bind(_ajax)
export const patch = _ajax.patch.bind(_ajax)
export const del = _ajax.del.bind(_ajax)
